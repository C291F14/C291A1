        ...
        while ans in confirm:
            ...
            if P1menuChoice == "1":
                ...
                try:
                    ...
                    EmployeeNumber = int(input("Enter 'Doctor' Employee Number: "))
                    DoctorName = input("Enter 'Doctor' First and Last Name. Format => 'John Doe': ")
                    TestName = input("Enter the Exact Test Name: ")
                    HealthCareNumber = int(input("Enter 'Patient' Health Care Number: "))
                    PatientName = input("Enter 'Patient' First and Last Name. Format => 'John Doe': ")
                except:
                    print("Invalid Input!\n")
                ...
                try:
                    ...
                    curs1.execute("SELECT type_id FROM test_type WHERE test_name = %s" %TestName)
                    # rows1 = (type_id)
                    ...
                    if rows1 == None:
                        ...
                        try:
                            ...
                            if NotAllowedCheck in rows2:
                                ...
                                insert_row(rows1, EmployeeNumber, DoctorName, TestName, HealthCareNumber, PatientName, con);
                                #break
...
def insert_row(TypeId, EmployeeNumber, DoctorName, TestName, HealthCareNumber, PatientName, con):

    # Getting the prescription date and converting it to be read by SQL.
    PrescribeDateStr = input("Enter Todays Date Which this Prescription was Created. Format => yyyy/mm/dd (complete with slashes): ").strip()
    PrescribeDate = cx.Date(int(PrescribeDateStr[:4]), int(PrescribeDateStr[5:7]), int(PrescribeDateStr[8:]))
    ...
    try:
        ...
        insert = """insert into test_record (test_id, type_id, patient_no, employee_no, medical_lab, result, prescribe_date, test_date) values (:test_id, :type_id, :patient_no, :employee_no, :medical_lab, :result, :prescribe_date, :test_date)"""
...
def prescription(con):
    ...
    try:
        ...
        while ans in confirm:
            ...
            if PmenuChoice == "1":
                try:
                    employee_no = input("Enter EMPLOYEE_NO of doctor: ")
                ...
                try:
                    ...
                    curs.execute("SELECT name FROM patient p, doctor d WHERE p.health_care_no = d.health_care_no;")
                    rows = curs.fetchall()
                    if doctor_name not in rows:
